...
const CHANNELS = [
  ...
  { icon: '💼', label: 'LinkedIn', value: 'linkedin.com/in/username',   href: 'https://linkedin.com/in/username' },
];
  ...
  useEffect(() => {
    [headerRef, leftRef, rightRef].forEach((ref, i) => {
      const observer = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting) { setTimeout(() => ref.current?.classList.add('visible'), i * 100); observer.disconnect(); }
      }, { threshold: 0.1 });
    ...
    <section id="kontak" className="section">
      ...
      <div className="contact-layout">
        <div className="reveal" ref={leftRef}>
          <p className="contact-intro">Punya ide proyek menarik atau ingin bekerja sama? Saya selalu terbuka untuk diskusi dan peluang baru.</p>
        ...
        <form className="contact-form reveal" ref={rightRef} onSubmit={handleSubmit} noValidate>
          {[['name','text','Nama Lengkap','Nama Anda'],['email','email','Alamat Email','email@anda.com'],['subject','text','Subjek','Tentang apa?']].map(([name,type,label,placeholder]) => (
            <div className="form-group" key={name}>
              <label className="form-label" htmlFor={name}>{label}</label>
              <input id={name} name={name} type={type} className="form-input" placeholder={placeholder} value={form[name]} onChange={handleChange} />
            </div>
          ))}
          <div className="form-group">
            <label className="form-label" htmlFor="message">Pesan</label>
            <textarea id="message" name="message" className="form-textarea" placeholder="Ceritakan lebih detail..." value={form.message} onChange={handleChange} />
          </div>
